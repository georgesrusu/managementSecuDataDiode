...
\hline
4 & Data diode hardware degradation & Low,Medium,High & This vulnerability could affect the three part of the information security(availability, integrity, confidentiality) and also lead to non-reversible damage. However, the impact time is according to the broken hardware component. \\
\hline
...
\hline
9 & Virus and malwares & Low, Medium, High & Depending of the type of the virus, the impact is as well depending on the type of the virus: a Trojan virus would not halt the system however confidentiality will be broken. \\
\hline
...
\hline
15 & Data credentials lost & Medium & If a user lose his credentials, he should immediately contact the administrator in order to reset his credential.\\
\hline
...
\begin{enumerate}
\item[-] Low: 1 time in 10 years.
\item[-] Medium: maximum 10 times per year.
\item[-] High: several time per month.
\end{enumerate}
...
\hline
3 & Data diode hardware failure & Medium,High & The data diode hardware are well chosen and tested in order to keep a low risk of failure, however hardware failure can happen. There is no perfect product to last forever in this world.\\ 
\hline
4 & data diode hardware degradation & Medium,High & A clumsiness is always possible (medium). Furthermore, if the access in the room of the data diode is not controlled, this attack is really simple and is likely to happen very often(high). \\
\hline
...
\hline
7 &  SQL injection & Skilled Hacker, script kiddies, unauthorized user  & In order to avoid the SQL injection, we must use in the code source of the web interface a function that prevents the interpretation of the SQL language. &H & H & High \\
\hline
8 & Brute force attack & & & & & \\
\hline
9 & Virus and malwares & Skilled hacker, script kiddies & This vulnerability is out of our scope. The client is responsible for securing access to internal network and so he have to use appropritate anti-virus and other. & H & H & High \\
\hline
10 & Misconfiguration & Administrator & The configuration is only possible through the administrator interface and by an adminsatrator hence, a administrator must be well chosen. & M & L & Low \\
\hline
11 & Failure in maintenance from the provider & Administrator & We test all the features of the data diode (such as the transfer) before providing it to the client. We make sure we always have an operation version. & H& L& Low\\
\hline
12 & Remote connection & Skilled Hacker & & & & \\
\hline
13 & Social engineering & Skilled Hacker, unauthorized user & Since it concerns the employees, the company should provide security awareness training in order to avoid social enginering. & H & M & High \\
\hline
...
\hline
15 &  Data credentials theft& Skilled Hacker, unauthorized user & In ordre to mitigate the risk of credential theft, the web interface must be available only thriugh https. & H & H & H \\
\hline
...
However, before starting, we should mention that most of the vulnerabilities nowadays are coming from the internet. Thus, our data diode should be exposed to such a danger. Despite the fact that the data diode could be installed within a LAN whit an internet connection, it should not be accesible form the internet, only from the LAN. can be located in a LAN which has a connection to the wolrd internet but 
%fin
\clearpage

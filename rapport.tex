...
\item[-] High: more than 10 times per month.
\end{enumerate}
...
\hline
2 & Fire & The data diode must be placed in a room protected against fire, closed with fire doors to mitigate as much as possible the risk of the data diode being affected by fire. & H & L & Low \\
\hline
3 & Data diode hardware failure & Since an administrator must be ready to intervene in case of a failure of the hardware, we can accept the risk of this vulnerability. & L & L & Low \\
\hline
...
\hline
5 & DDOS attack & & H & H & High \\
\hline
6 & Zero Day attack & Due to the fact that we can't be prepare against this attack, we accept the risks of this vulnerability. & H & M & Medium \\
\hline
7 &  SQL injection & In order to avoid the SQL injection, we must use in the code source of the web interface a function that prevents the interpretation of the SQL language. &H & H & High \\
\hline
8 & Brute force attack & & & & \\
\hline
9 & Virus and malwares & This vulnerability is out of our scope. The client is responsible for securing access to internal network and so he have to use appropritate anti-virus and other. & H & H & High \\
\hline
10 & Misconfiguration & The configuration is only possible through the administrator interface and by an adminsatrator hence, a administrator must be well chosen. & M & L & Low \\
\hline
11 & Failure in maintenance from the provider & We test all the features of the data diode (such as the transfer) before providing it to the client. We make sure we always have an operation version. & H& L& Low\\
\hline
12 & Remote connection & Skilled Hacker & & & \\
\hline
13 & Social engineering & Since it concerns the employees, the company should provide security awareness training in order to avoid social enginering. & H & M & High \\
\hline
14 & Data credentials lost & Having more than one administrator is recommended but not an imperative countermeasure & M & L & Low \\
\hline
15 & Data credentials theft & In ordre to mitigate the risk of credential theft, the web interface must be available only thriugh https. & H & H & H \\
\hline

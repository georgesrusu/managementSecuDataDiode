...
\begin{enumerate}
\item[-] uncontrollable or inevitable events.
\item[-] controllable or avoidable events.
\end{enumerate}

\subsubsection{Uncontrollable events}
Such events could be related to natural disaster which cannot be predicted in advance. Thus, there is no way in which to prevent the mass destruction. Such events could be related to natural disaster. This kind of disaster comprise:
\begin{enumerate}
\item[-] Earthquakes
\item[-] Storms
\item[-] Floods
...
\item[-] Wildfires
\end{enumerate}
However, some event could have a much higher probability of appearance depending on the territory where the company has his headquarters. So the company could try to prevent some of those by trying for instance to 
Despite the fact there can be much more natural disaster, the 
